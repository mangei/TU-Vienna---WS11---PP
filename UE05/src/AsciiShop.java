						...
						if(imageWidth > 0) {
							...
							if(scanner.hasNextInt()) {
								...
								if(imageHeight > 0) {
									...
									image = new AsciiImage(imageWidth, imageHeight);
						...
						if(validLoad) {
							...
							for(int y=0; y<imageHeight; y++) {
								for(int x=0; x<imageWidth; x++) {
									image.setPixel(x, y, loadImage[y].charAt(x));
								}
							}
								...
								if(scanner.hasNextInt()) {
									...
									if(scanner.hasNext()) {
										...
										if(cStr.length() == 1) {
											...
											// check attributes
											if(x0 >= 0 && x1 >= 0 && 
												x0 < image.getWidth() && x1 < image.getWidth() &&
												y0 >= 0 && y1 >= 0 &&
												y0 < image.getHeight() && y1 < image.getHeight()) {
